...
def decompress_audio(algorithm='shannon_fano'):
    import shutil
    ...
    compressor = AudioShannonCompressor()
    ...
    try:
        ...
        decoded_data = compressor.shannon_fano_decode(compressed_data, metadata)
...
class AudioShannonCompressor:
    ...
    def compress_audio_file(self, input_path, output_path):
        ...
        try:
            ...
            with open(input_path, 'rb') as f:
                data = f.read()
            samples = np.frombuffer(data, dtype=np.uint8).astype(np.float32) / 255.0
        
        compressed_data, metadata = self.shannon_fano_encode(samples)
    ...
    def decompress_audio_file(self, input_path, output_path):
        with open(input_path, 'rb') as f:
            compressed_data = f.read()
        ...
        if hasattr(decoded_data, 'tofile'):
            decoded_data.tofile(output_path)
        else:
            with open(output_path, 'wb') as f:
                f.write(decoded_data)
...
def compress_audio(algorithm='shannon_fano'):
    ...
    try:
        ...
        try:
            ...
            samples = np.array(audio.get_array_of_samples(), dtype=np.float32)
            samples = samples / (2 ** 15)
            compressed_data, metadata = compressor.shannon_fano_encode(samples)
        except:
            compressed_data, metadata = compressor.shannon_fano_encode(data)
        
        output_file = f"{outputShannonAudio}/{os.path.splitext(os.path.basename(selected_audio))[0]}.sf"
...
def compressAudioFile(algorithm='shannon_fano'):
    compress_audio(algorithm)


def decompressAudioFile(algorithm='shannon_fano'):
    decompress_audio(algorithm)
